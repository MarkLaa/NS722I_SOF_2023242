...
namespace Féléves_Szerveroldali.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
            ...
            else
            {
                ...
                if (info.Principal.HasClaim(c => c.Type == ClaimTypes.Email))
                {
					...
					Input = new InputModel
                    {
						...
						PictureUrl = "http://graph.facebook.com/" + info.Principal.FindFirstValue(ClaimTypes.NameIdentifier) + "/picture?type=square"
					};
                    var access_token_json = new WebClient().DownloadString("http://graph.facebook.com/oauth/access_token?client_id=1015500313252537&client_secret=d16ec70dc761954b7c48ae280be2796c&grant_type=client_credentials");
                    var token = JsonConvert.DeserializeObject<TokenModel>(access_token_json);
                    Input.PictureUrl = $"http://graph.facebook.com/{id}/picture?type=large&access_token={token.access_token}";
